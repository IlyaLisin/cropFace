...
void FaceImageProcessing::setImagePath(std::string path) {
    _path = new char[path.length()+1];

    strcpy(_path, path.c_str());
}

void FaceImageProcessing::setHaarcascadePath(std::string hcPath) {
    _hcPath = new char[hcPath.length()+1];

    strcpy(_hcPath, hcPath.c_str());
}

void FaceImageProcessing::foundFacePoints(float* landmarks) {
	// read image and convert
    cv::Mat_<unsigned char> img(cv::imread(_path, CV_LOAD_IMAGE_GRAYSCALE));
	...
	if (!stasm_search_single(&foundface, landmarks,
        (char*)img.data, img.cols, img.rows, _path, _hcPath))
	{
...
cv::Mat q = imgOut(r);
    ...
    multiply(imgOut(r), cv::Scalar(1.0, 1.0, 1.0) - mask, q);
